...
from joint_control.keyframes import hello, leftBackToStand, rightBellyToStand
...
class PostHandler(object):
    ...
    def execute_keyframes(self, keyframes):
        ...
        # YOUR CODE HERE
        thread = threading.Thread(target=self.client.execute_keyframes(keyframes), args=[keyframes])
        thread.start()

    def set_transform(self, effector_name, transform):
        '''non-blocking call of ClientAgent.set_transform'''
        # YOUR CODE HERE
        thread = threading.Thread(target=self.client.set_transform(effector_name, transform),
